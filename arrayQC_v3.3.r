...
#####Change Flags#####################################
 ...
 if (spikes==TRUE)
 {
  gpr$other$Name[grep('EC', gpr$genes$ID)]=gpr$genes$ID	        #Spikes have no Name on the 8x15k array -> get them back.
 } 
...
if (global !="none")
{
 ...
 if (ratio.form=="Median of Ratios (532/635)")
 {
  norm.ratios=1/norm.ratios
  cat("Ratio will be swapped\n")
 }
 
 ##Corrected for correct plotting with dye swap##
 MA.global$M=log2(norm.ratios)
 ##
 ...
 ##
 MA.global.only=MA.global
 MA.global.only$M=log2(norm.ratios)
 gpr.filtered.only$other$Normalize=norm.ratios
 ##
 ...
 if (local=="inHouse")
 {
  ...
  data.local=cbind(gpr.filtered.only$genes$Row,gpr.filtered.only$genes$Column,norm.ratios)
...
#
  plotMA(gpr.median,main="MA plot of filtered centered data",ylim=c(-10,10))
...
#  
  ...
  if((global != "none") | (local == "inHouse") | (local == "ma2D"))
  {
   plotMA(MA.global.only,main="MA plot after global Nomalisation", ylim=c(-10,10))
   abline(h=c(log2(2),-log2(2)), col="red",lwd=1.5)
   ...
   plotMA(MA.global,main="MA plot after local Normalisation", ylim=c(-10,10))
   abline(h=c(log2(2),-log2(2)), col="red",lwd=1.5)
   plot(gpr$other$X,log2(norm.all),ylim=c(-4,4),pch=20,cex=0.7,col="grey", xlab="X position on the array", ylab="Normalised log2 ratios")
...
######Compute gene names stats on good genes ########################################################
 ...
 if (tiling==FALSE)
 {
  ...
  #name=unique(gpr$genes$Name)

  len.name=length(unique(gpr$genes$Name))
...
########Display stats############################
 if (verbose==TRUE)
 {
  ...
  {
   cat(
   "Number of genes/features/controls discarded: ",(len.name-length(name)),"\n",
   sep=''
   )
